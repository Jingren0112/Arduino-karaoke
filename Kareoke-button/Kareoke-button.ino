            ...
            if (melody[i][thisNote] == 1) {
              ...
              if (divider > 0) {
               ...
               for (int x = thisNote; x > 0; x--) {
                ...
                tone(BUZZER, melody[i][thisNote], noteDuration * 0.9);// Wait for the specief duration before playing the next note.
                delay(noteDuration);// stop the waveform generation before the next note.
                noTone(BUZZER);
                thisNote++;
              } else {
                thisNote = cols;
